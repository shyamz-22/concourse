...
var _ = Describe("Pool", func() {
	...
	Describe("RunTaskStep", func() {
		var (
			...
			fakeWorkerSpec       WorkerSpec
			fakeContainerSpec    ContainerSpec
			fakeStrategy         *workerfakes.FakeContainerPlacementStrategy
			fakeMetadata         db.ContainerMetadata
			fakeDelegate		 *execfakes.FakeTaskDelegate
			fakeImageFetcherSpec ImageFetcherSpec
			fakeTaskProcessSpec  TaskProcessSpec
			fakeContainer *workerfakes.FakeContainer
			fakeVolume *workerfakes.FakeVolume
			eventChan chan string
		)
		JustBeforeEach(func() {
			 status, volumeMounts, err = pool.RunTaskStep(
				context.TODO(),
				logger,
				fakeContainerOwner,
				fakeContainerSpec,
		...
		BeforeEach(func() {
			...
			fakeProvider.FindWorkersForContainerByOwnerReturns([]Worker{fakeWorker}, nil)
		...
		Context("choosing a worker", func() {
			...
			It("chooses a worker", func() {
				...
				Expect(fakeProvider.FindWorkersForContainerByOwnerCallCount()).To(Equal(1))
			})
		})
		...
		Context("found container already exited", func() {
			...
			It("returns result of container process", func() {
				...
				Expect(volumeMounts).To(ConsistOf(VolumeMount{
					Volume: fakeVolume,
					MountPath:"some/path",
				}))
